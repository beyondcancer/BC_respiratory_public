...
review_vars <- data.frame(
  cancer = character(),
  outcome = character(),
  model_type = character(),
  ph_test = character(),
  events_exp_adj = numeric(),
  events_unexp_adj = numeric(),
  total_fup_exp_adj = numeric(),
  total_fup_unexp_adj = numeric(),
  stringsAsFactors = FALSE)
...
for(site in cancersites) {
  ...
  print(paste0("Initiating loop for "))
    ...
    #Count total follow-up time in exposed and unexposed group and save in a variable
    total_fup_exp  <- sum(cr_outcome_an[["total_fup_var"]][cr_outcome_an$exposed == 1])
    total_fup_unexp <- sum(cr_outcome_an[["total_fup_var"]][cr_outcome_an$exposed == 0])
    ...
    # Check if the model ran successfully or not
      ...
      print(summary(cox_model_adj))
        ...
        ph_p_value <- tryCatch({
          ph_test <- cox.zph(cox_model_adj)
          ph_test$table["exposed", "p"]
        }, error = function(e) {
      ...
      review_vars <- rbind(review_vars, data.frame(
        cancer = site,
        outcome = outcome,
        model_type = "Adjusted",
        ph_test = ph_p_value,
        events_exp_adj = n_exp,
        events_unexp_adj = n_unexp,
        total_fup_exp_adj = total_fup_exp,
        total_fup_unexp_adj = total_fup_unexp,
        stringsAsFactors = FALSE))
      ...
      ##Convert to factor variables
          ...
          adjusted_params_split <- as.formula(
            paste("Surv(start_time, total_fup_var,", event_var, ") ~ exposed:survival + survival + strata(setid) +", paste(covariates, collapse = "+"))
          )
      ...
      if (is.null(cox_model_split)) {
        ...
        ci_upper <- x$conf.high
          ...
          for (period in 1:4) {
            ...
            period_data <- split_survival %>%
              filter(survival == as.character(period))
            
            period_label <- c("0-1 years", "1-3 years", "3-5 years", "5+ years")[period]
            
            new_row <- data.frame(
              cancer = site,
              outcome = outcome,
              model_type = period_label,
              ph_test = NA,  # not applicable for time-split models
              events_exp_adj = sum(period_data[[event_var]][period_data$exposed == 1]),
              events_unexp_adj = sum(period_data[[event_var]][period_data$exposed == 0]),
              total_fup_exp_adj = sum((period_data$total_fup_var - period_data$start_time)[period_data$exposed == 1]),
              total_fup_unexp_adj = sum((period_data$total_fup_var - period_data$start_time)[period_data$exposed == 0]),
              stringsAsFactors = FALSE
            )
            
            review_vars <- rbind(review_vars, new_row)
          }
...
rownames(review_vars) <- NULL
#save summary tables as rds

saveRDS(results, file = paste0(path_datafiles_for_analysis, "rev/an_cox_models_first_dx_crude_adj.rds"))

 write.csv(review_vars, file = paste0(path_datafiles_for_analysis, "rev/prop_hazards_inc_rate.csv"))
