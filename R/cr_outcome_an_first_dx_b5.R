...
for(site in cancersites) {
  ...
  for (outcome in outcomes_chronic) {
    ...
    date_history_var <- paste0("dol_b_", outcome)
    ...
    #EXCLUDE THOSE WITH a Dx RECORD of outcome less than 5 years before indexdate
    cr_cancer_data <- cr_cancer_data %>% 
      mutate(time_since_b = as.numeric(indexdate - .data[[date_history_var]]) / 365.25)  # Convert to years
    
    cr_cancer_data <- cr_cancer_data %>% 
      mutate(b_out_5y = if_else(time_since_b >= 5, 1, 0)) 
    
    count_hx5_yrs <- sum(cr_cancer_data[[history_var]] == 1 & cr_cancer_data$b_out_5y == 1, na.rm = TRUE)

  
    print(paste0("People with history of outcome record: ", count_with_history))
    print(paste0("People with a history of outcome less than 5 years before indexdate: ", count_with_history-count_hx5_yrs))
    print(paste0("percent of total that have b_asthma = 1: ", ((count_with_history-count_hx5_yrs)/nrow(cr_cancer_data) * 100)))
    
    
    cr_outcome_an <-cr_cancer_data %>%
    dplyr::mutate(!!sym(history_var) := ifelse(.data[[history_var]] == 1 & b_out_5y == 1, 0, .data[[history_var]]))

    
    count_with_history_unexp <- sum(cr_outcome_an[[history_var]] == 1 & cr_cancer_data$exposed == 0)
    count_with_history_exp <- sum(cr_outcome_an[[history_var]] == 1 & cr_cancer_data$exposed == 1)
    ...
    cr_outcome_an <- cr_outcome_an %>%
      filter(!!sym(history_var) == 0)  # exclude those with a history of outcome
    ...
    file_name <- paste0(path_datafiles_for_analysis, "sens2/", site, "_", outcome, "_cr_outcome_an_b5.csv")
...
write.csv(population, paste0(path_results, "/first_dx_analysis_numbers_b5.csv"), row.names = FALSE)
