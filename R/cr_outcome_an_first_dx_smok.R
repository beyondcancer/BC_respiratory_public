...
for(site in cancersites) {
  ...
  for (outcome in outcomes_chronic) {
    ...
    print(paste0("People with a history of outcome or having ever smoked: ", count_with_history))
    ...
    cr_outcome_an <- cr_cancer_data %>%
      filter(.data[[history_var]] == 0 & smokstatus == 0)  # exclude those with a history of outcome
    ...
    file_name <- paste0(path_datafiles_for_analysis, "sens4/", site, "_", outcome, "_cr_outcome_an_smok.csv")
...
write.csv(population, paste0(path_results, "/first_dx_analysis_numbers_smok.csv"), row.names = FALSE)
