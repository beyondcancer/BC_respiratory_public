...
for(site in cancersites) {
  ...
  for (outcome in outcomes_chronic) {
    ...
    cr_outcome_an <- read.csv(paste0(path_datafiles_for_analysis,"sens4/", site, "_", outcome, "_cr_outcome_an_smok.csv"))
    
    #Create dynamic variables  
    history_var <- paste0("b_", outcome)  # e.g., b_asthma
    event_var <- paste0("any_", outcome, "_inc_dx")  # e.g., asthma_inc_dx
    
    covariates <- c("imd5", "bmi","b_asthma", "b_copd", "b_bronch", "b_ild", "b_ckd", "b_hypertension", "b_cvdgrouped", "b_diab_cat", "b_cld", "b_multiple_sclerosis")
...
saveRDS(results, file = paste0(path_datafiles_for_analysis, "sens4/an_cox_models_first_dx_crude_adj_smok.rds"))
