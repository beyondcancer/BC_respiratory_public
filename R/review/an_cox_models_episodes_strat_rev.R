...
results <- data.frame(
  ...
  for (outcome in outcomes_chronic) {
    ...
    for (site in cancersites) {
      ...
      factors <- c("exposed", "age_up_cat", "imd5", "bmi_cat", "smokstatus", "gender", "region", "eth5_hes")
    ...
    smok_strat_params <- as.formula(paste0("Surv(tstart, time, episode) ~ exposed:smokstatus + cluster(e_patid) +", paste(covariates, collapse = "+")))
...
saveRDS(results, file = paste0(path_datafiles_for_analysis, "/rev/an_cox_models_episodes_strat_smok.rds"))
write.csv(lrt_results, file = paste0(path_results, "/rev/lrtresults_exc_smok.csv"))
