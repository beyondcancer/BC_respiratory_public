...
for(site in cancersites) {
  ...
  for (outcome in outcomes_chronic) {
    ...
    smok_strat_params <- as.formula(paste("Surv(total_fup_var,", event_var, ") ~ exposed:smokstatus  + strata(setid) + ", paste(covariates, collapse = "+")))
...
saveRDS(results, file = paste0(path_datafiles_for_analysis, "/rev/an_cox_models_first_strat_smok.rds"))
 write.csv(lrt_results, file = paste0(path_results, "/rev/lrtest_results_smok.csv"))
