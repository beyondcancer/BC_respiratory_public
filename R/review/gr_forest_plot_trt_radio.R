...
# results_chemo <-  results_gph %>% filter(study == "Adjusted" | study == "Chemo")
results <- readRDS(results, file = paste0(path_datafiles_for_analysis, "an_cox_models_first_dx_trt.rds"))

#create variable study with the first word of model type
results <- results %>% mutate(study = word(model_type, 1))
results <- results %>%
  mutate(model_type = case_when(
    model_type == "Chemo Treatment" ~ "Chemotherapy",
    model_type == "Chemo No Treatment" ~ "No Chemotherapy",
    model_type == "Radio Treatment" ~ "Radiotherapy",
    model_type == "Radio No Treatment" ~ "No Radiotherapy",
    TRUE ~ model_type
  ))
...
results_radio <-  results %>% filter(study ==  "Radio" & (cancer == "bre" | cancer == "oes"| cancer == "nhl"| cancer == "lun"))
  ...
  xmin <- 0.25 #CI limits
  xmax <- 11 #CI limits
  x_label_col <- 10.1 # x position for HR text labels
...
#PLOT
  ...
  p <- ggplot() +
    ...
    coord_cartesian(xlim = c(0.25, 85)) +
    ...
    scale_color_manual(
      values = color_map,
      name   = "Treatment",
      labels = c("No Radiotherapy" = "No radiotherapy", "Radiotherapy" = "Radiotherapy"),
      guide  = guide_legend(reverse = TRUE)
    ) +
    scale_shape_manual(
      values = c("No Radiotherapy" = 16, "Radiotherapy" = 15),
      name   = "Treatment",
      labels = c("No Radiotherapy" = "No radiotherapy", "Radiotherapy" = "Radiotherapy"),
      guide  = guide_legend(reverse = TRUE)
    ) +
